...
- **Power Rankings Service** (`power-rankings-service.ts`): Dynasty Dominance Score (DDS) V3 Engine — `DDS = (RosterEV × w1) + (PickEV × w2) + (Depth × w3) + (Liquidity × w4) + ((100 - Risk) × w5)` with projection-window-adaptive weights (1Y/3Y/5Y). All sub-metrics normalized 0-100 across league. Pick EV uses historical hit-rate curves per slot. Multi-factor risk engine (injury, age cluster, concentration, volatility). Concentration index (top-3 asset share). Strategy classifier (Elite Contender/Ascending Builder/Fragile Contender/Liquid Rebuilder/Balanced/Full Rebuild). Weekly movement delta tracking via `power_ranking_snapshots` DB table (auto-snapshot every 7 days). Softmax championship & playoff probability. Tier classification: Elite Contender (>=80), Contender (70-79), Competitive (60-69), Retool (45-59), Rebuild (<45). Age grading (A+ through C) using value-weighted core+starter age. No season-stat dependency — works fully in offseason as "Dynasty Projection Mode". Both main route (`/api/sleeper/power-rankings?window=3`) and v3 service use the same formula.
