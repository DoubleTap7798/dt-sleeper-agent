...
- **AI Manager Profile**: AI-learned manager personality system (`/league/manager-profile`). Analyzes up to 3 seasons of trade history, waiver moves, and transaction patterns per league. Builds a profile (style, risk tolerance, age preference, position preferences, trade frequency, draft pick strategy, waiver activity, key patterns, strengths, blind spots). Profile context is injected into AI Chat and Trade Analyzer for personalized recommendations.
