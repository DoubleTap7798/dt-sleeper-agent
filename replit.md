...
- **Power Rankings Service** (`power-rankings-service.ts`): Dynasty Composite Engine — `calculateDynastyComposite(team)` = RosterEV + FuturePickEV + AgeCurveAdjustment + DepthScore + LiquidityScore - RiskPenalty. Normalized 0-100 across league. Softmax championship probability. Tier classification: Elite Contender (>=80), Contender (70-79), Competitive (60-69), Retool (45-59), Rebuild (<45). No dependency on season stats — works fully in offseason as "Dynasty Projection Mode". Age grading (A through C), risk profiling (Low/Moderate/Elevated/High). Both main route (`/api/sleeper/power-rankings`) and v3 service use the same formula.
