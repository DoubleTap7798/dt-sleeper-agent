...
**Draft Intelligence Engine** — Multi-source institutional-grade draft pricing from Sleeper community data and expert consensus rankings:
- **Service** (`server/engine/draft-intelligence-service.ts`): Ingests drafts from all user Sleeper leagues, classifies type (startup/rookie) and format (1QB/SF/TEP), computes ADP (overall + per format), builds pick value curves (avg dynasty value at each pick slot), and maps every player to rookie/startup pick equivalents. Blends Sleeper community ADP with DynastyProcess expert consensus rankings (ECR) using a sample-size-weighted formula: `consensus = adpRank * w + ecrRank * (1-w)` where `w = min(samples/20, 1)`. **Minimum 3 draft samples required** (HAVING COUNT >= 3). Players with Sleeper data but NO ECR match receive a confidence penalty: `penalty = 200 * (1 - min(samples/20, 1))` — pushes low-sample unranked players far down the list. Query-level safety: `sample_size >= 3 OR data_sources LIKE '%dynastyprocess%'`. Full pipeline: `fetchDynastyProcessRankings()` → `ingestAllDrafts()` → `computeADP()` → `computePickValueCurve()` → `computePickEquivalents()`.
- **External Rankings Service** (`server/engine/external-rankings-service.ts`): Fetches DynastyProcess CSV (750 players, updated weekly) with ECR for 1QB and SF formats. Matches players to Sleeper IDs via normalized name+position matching. Stored in `external_rankings` table.
- **Database**: `drafts` table (draft_id PK, league_id, type, format, status, rounds, teams, season). `draft_picks` table (draft_id, round, pick_no, player_id, player_name, position). `draft_adp` table (player_id, adp_overall, adp_1qb, adp_sf, adp_tep, sample sizes, rookie_pick_eq, startup_pick_eq, ecr_1qb, ecr_sf, consensus_rank, data_sources, draft_type). `pick_value_curve` table (pick_number, draft_type, avg_dynasty_value, sample_size). `external_rankings` table (player_name, position, team, source, ecr_1qb, ecr_sf, value_1qb, value_sf, sleeper_id). **`draft_type` column**: 'rookie' if player is a 2026 prospect (DynastyProcess-only with team='FA' or no team), 'startup' otherwise. All Sleeper-drafted players are 'startup' (current NFL players). External-only DynastyProcess players with an NFL team are 'startup'.
- **Background Jobs**: Full pipeline 60s after startup + every 6 hours via setInterval. Rate-limited Sleeper API calls (250ms delay). DynastyProcess CSV fetched at start of each pipeline run.
- **API Routes**: `GET /api/draft-intelligence/adp` (paginated, filterable by format/position/search/draftType, sort=consensus|sleeper|ecr), `GET /api/draft-intelligence/adp/:playerId`, `GET /api/draft-intelligence/pick-value-curve` (query: type=rookie|startup, enhanced=true for player names), `GET /api/draft-intelligence/sources` (data source summary with draft/pick counts), `POST /api/draft-intelligence/refresh` (manual trigger). **Default category=offense** filters to QB/RB/WR/TE only; pass category=all for IDP. **draftType** param: 'all' (default), 'rookie' (2026 draft class only), 'startup' (dynasty startup players). ORDER BY uses whitelisted column names to prevent SQL injection. Market heat data (market_heat_level, market_label, hype_velocity) joined from player_market_metrics.
- **Frontend**: Data source indicator badges (amber=Sleeper, green=DynastyProcess), sort dropdown (Consensus/Sleeper ADP/Expert ECR), ECR column alongside ADP, source dots per player row, Data Sources card in overview. Market heat icons inline per player (Flame=HOT, TrendingUp=HEATING, Snowflake=COLD, TrendingDown=COOLING). Pick Value Curve shows explainer card with draft count, most-common-player per pick slot, % of top value, color-coded bars. Rookie mode uses raw frequency (same player can appear at multiple picks); Startup mode defaults to "Unique Pick Leader" algorithm (each player appears once, assigned to their highest-frequency pick slot) with toggle to switch to raw frequency. Clicking a player opens a mini card showing total times drafted, average pick, median pick, full pick distribution with bars, and league format breakdown (1QB/SF/TEP). Sources with 0 players hidden; Sleeper shows draft count instead of player count.
