...
**Draft Intelligence Engine** — Institutional-grade draft pricing from real Sleeper draft data:
- **Service** (`server/engine/draft-intelligence-service.ts`): Ingests drafts from all user Sleeper leagues, classifies type (startup/rookie) and format (1QB/SF/TEP), computes ADP (overall + per format), builds pick value curves (avg dynasty value at each pick slot), and maps every player to rookie/startup pick equivalents. Full pipeline: `ingestAllDrafts()` → `computeADP()` → `computePickValueCurve()` → `computePickEquivalents()`.
- **Database**: `drafts` table (draft_id PK, league_id, type, format, status, rounds, teams, season). `draft_picks` table (draft_id, round, pick_no, player_id, player_name, position). `draft_adp` table (player_id, adp_overall, adp_1qb, adp_sf, adp_tep, sample sizes, rookie_pick_eq, startup_pick_eq). `pick_value_curve` table (pick_number, draft_type, avg_dynasty_value, sample_size).
- **Background Jobs**: Full pipeline 60s after startup + every 6 hours via setInterval. Rate-limited Sleeper API calls (250ms delay).
- **API Routes**: `GET /api/draft-intelligence/adp` (paginated, filterable by format/position/search), `GET /api/draft-intelligence/adp/:playerId`, `GET /api/draft-intelligence/pick-value-curve` (query: type=rookie|startup), `POST /api/draft-intelligence/refresh` (manual trigger).
