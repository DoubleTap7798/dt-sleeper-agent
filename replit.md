...
- **API Routes**: `GET /api/draft-intelligence/adp` (paginated, filterable by format/position/search, sort=consensus|sleeper|ecr), `GET /api/draft-intelligence/adp/:playerId`, `GET /api/draft-intelligence/pick-value-curve` (query: type=rookie|startup), `GET /api/draft-intelligence/sources` (data source summary), `POST /api/draft-intelligence/refresh` (manual trigger).
- **Frontend**: Data source indicator badges (amber=Sleeper, green=DynastyProcess), sort dropdown (Consensus/Sleeper ADP/Expert ECR), ECR column alongside ADP, source dots per player row, Data Sources card in overview.
