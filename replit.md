...
Core features include career stats, standings, news feeds, matchups, lineup advice, schedule, playoff bracket, rivalries, waiver wire, player watchlist, smart trade ideas, NFL players, player profiles, player trends, comparisons, ROS projections, Devy Command Center, 2026 Draft Board (probability engine with EV scores, elite/bust rates, tier cliff detection, position-specific historical modeling, and cross-linked pick values), Trade Lab, Trade History, Trophy Room, AI Chat Assistant, real-time notifications, power rankings, lineup optimizer, export functionality, draft pick value charts (with positional filters, prospect-per-pick context, strategy panels, and trade EV deltas), league history timeline, NFL stat leaders, share functionality, mobile navigation, strength of schedule analysis, season-long projections, usage trends, and injury tracker.
...
**Dynasty AI Engine v3** (server/engine/) provides the quantitative analytics backbone:
- **Player Valuation Service** (`player-valuation-service.ts`): DNPV engine with 7-year value curves, position-specific age curves, longevity modifiers, injury risk modeling, archetype clustering, and production trajectory analysis. Values cached for 5 minutes.
- **Devy Projection Service** (`devy-projection-service.ts`): Probabilistic NFL Draft outcome distributions, branch-weighted dynasty values, contract arc modeling, and risk-adjusted valuations.
- **Franchise Modeling Service** (`franchise-modeling-service.ts`): Window classification (Win Now/Contender/Balanced/Productive Struggle/Rebuild), Window Score calculation, age-weighted production, draft capital reserves, depth insulation scoring, and 2-year championship probability.
- **UFAS Service** (`ufas-service.ts`): Unified Franchise Asset Score — the central scoring currency combining base talent, longevity, window fit, positional scarcity, risk adjustment, and market demand. All modules use UFAS as the single scoring backbone.
- **Draft Simulation Service** (`draft-simulation-service.ts`): Monte Carlo simulator (1000+ iterations), opponent need modeling, tier drop-off analysis, Draft Leverage Score, availability probabilities, and draft-now vs wait EV calculations.
- **Trade Simulation Service** (`trade-simulation-service.ts`): 5-year outcome distributions with Monte Carlo, immediate/3-year/5-year EV deltas, bust probability, championship probability delta, volatility scoring, window alignment, and structured counter-offers with verdicts.
- **Market Dynamics Service** (`market-dynamics-service.ts`): Positional scarcity index, manager trading profiles, risk profiles, pick hoarding detection, contender/rebuild distribution, and proactive trade target generation.
- **Power Rankings Service** (`power-rankings-service.ts`): Dynasty Dominance Score (DDS) V3 Engine — `DDS = (RosterEV × w1) + (PickEV × w2) + (Depth × w3) + (Liquidity × w4) + ((100 - Risk) × w5)` with projection-window-adaptive weights (1Y/3Y/5Y). All sub-metrics normalized 0-100 across league. Pick EV uses historical hit-rate curves per slot. Multi-factor risk engine (injury, age cluster, concentration, volatility). Concentration index (top-3 asset share). Strategy classifier (Elite Contender/Ascending Builder/Fragile Contender/Liquid Rebuilder/Balanced/Full Rebuild). Weekly movement delta tracking via `power_ranking_snapshots` DB table (auto-snapshot every 7 days, with `mode` column for dynasty/in-season differentiation). Softmax championship & playoff probability. Tier classification: Elite Contender (>=80), Contender (70-79), Competitive (60-69), Retool (45-59), Rebuild (<45). Age grading (A+ through C) using value-weighted core+starter age. No season-stat dependency — works fully in offseason as "Dynasty Projection Mode".
- **In-Season Rankings Service** (`in-season-rankings-service.ts`): Competitive-strength model with 6 normalized sub-metrics: Starter PPG (25%), Win% (20%), Points Above Median (15%), All-Play Record (15%), 3-Week Momentum (15%), Injury-Adjusted Depth (10%). All metrics normalized 0-100 across league. Tier classification: Elite (>=85), Strong (>=70), Average (>=50), Below Average (>=35), Weak (<35). Strategy labels: Hot Streak, Consistent Contender, Underperformer, Overachiever, Surging, Rebuilding. Offseason detection returns empty results. Softmax championship odds.
- **Dual Power Rankings System**: Route `GET /api/sleeper/power-rankings/:leagueId?mode=dynasty|in-season|hybrid&window=1|3|5`. Dynasty mode (default): existing DDS with window toggle. In-Season mode: competitive-strength metrics from weekly matchup data. Hybrid mode: `0.6 * inSeasonScore + 0.4 * dynastyDDS(1Y)` blend. Frontend toggle between all 3 modes. Separate DB snapshots per mode. Offseason gracefully falls back to dynasty mode.
- **AI Explanation Service** (`ai-explanation-service.ts`): GM-briefing-style structured explanations with tier context, window alignment, scarcity leverage, risk profiles, and multi-year EV impact. Replaces generic LLM-based explanations with pre-computed structured data briefings.
- **3-Year Projection Model** (`projection-model.ts`): `compute3YearProjection()` with position-specific aging models (RB cliff 27, WR peak 25-29, QB longevity 24-34, TE peak 25-30), production-weighted base PPG, usage adjustments, and memoized results (15-min TTL).
- **API Routes**: All v3 engine endpoints under `/api/engine/v3/` with 5-minute caching via `engineV3Cache`. Key unified endpoints:
  - `GET /api/engine/v3/player-card/:playerId` — Unified dynasty player card (snapshot, multi-season stats, trends, projections, context). 10-min cache, 500-entry max.
  - `GET /api/engine/v3/players-dynasty` — Paginated/sortable dynasty player list with precomputed rollups (dynasty value, 3Y avg PPG, projected PPG, risk score, UFAS). 15-min cache. Supports server-side sort/filter/pagination.
