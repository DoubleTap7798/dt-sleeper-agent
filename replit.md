...
- **Database**: `drafts` table (draft_id PK, league_id, type, format, status, rounds, teams, season). `draft_picks` table (draft_id, round, pick_no, player_id, player_name, position). `draft_adp` table (player_id, adp_overall, adp_1qb, adp_sf, adp_tep, sample sizes, rookie_pick_eq, startup_pick_eq, ecr_1qb, ecr_sf, consensus_rank, data_sources). `pick_value_curve` table (pick_number, draft_type, avg_dynasty_value, sample_size). `external_rankings` table (player_name, position, team, source, ecr_1qb, ecr_sf, value_1qb, value_sf, sleeper_id).
