...
- **Frontend**: Data source indicator badges (amber=Sleeper, green=DynastyProcess), sort dropdown (Consensus/Sleeper ADP/Expert ECR), ECR column alongside ADP, source dots per player row, Data Sources card in overview. Market heat icons inline per player (Flame=HOT, TrendingUp=HEATING, Snowflake=COLD, TrendingDown=COOLING). Pick Value Curve shows explainer card, most-common-player per pick slot, % of top value, color-coded bars. Sources with 0 players hidden; Sleeper shows draft count instead of player count.
